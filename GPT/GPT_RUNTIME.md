...
6. when first analyzing a vacancy, populate `Company`, `Position`, `Archetype`, `Location`, `Vacancy URL`, `Date found`, `Fit %`, `Stage`, and `Next action` when known; if no CV exists yet, set `Stage = Reviewed`;
...
10. store exactly four artifacts in the position folder: `Position.md` with vacancy metadata and full text; `Anton_Nazarov<PositionTitle>.md` as the tailored CV Markdown source; `Anton_Nazarov_<PositionTitle>.docx` as the final Word CV; and `Anton_Nazarov<PositionTitle>.txt` as the plain-text cover letter;
...
- Keep the fixed columns `Company`, `Position`, `Archetype`, `Location`, `Vacancy URL`, `Date found`, `Date applied`, `Fit %`, `Stage`, `Last contact`, `Next action`, `CV`, `Cover Letter`, `Vacancy snapshot`, `Notes`, and `Vacancy text`.
...
- On first analysis, write all known vacancy fields and enough `Vacancy snapshot` / `Notes` / `Vacancy text` content to identify the role if its source page disappears; if no CV exists, set `Stage = Reviewed`.
...
- Store exactly four files in that position folder: `Position.md`, `Anton_Nazarov<PositionTitle>.md`, `Anton_Nazarov_<PositionTitle>.docx`, and `Anton_Nazarov<PositionTitle>.txt`. The files are respectively the vacancy source, CV Markdown, final Word CV, and plain-text cover letter.
...
This folder is the flat project bundle for ChatGPT Project usage.
...
Use files selectively.
Do not assume every task requires every file in its mode.
Short tasks should use the minimum context needed.

## Important note about canonical sources

These are project copies for portability.

Canonical originals still live in:
