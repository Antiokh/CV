...
- Use `work-application-manager/SKILL.md`, then load `work-application-manager/references/tracker-storage-v5.md`, `work-application-manager/references/salary-normalization-v6.md`, `work-application-manager/references/cv-markdown-v2.md`, and `work-application-manager/references/activity-log.md`. The v5 tracker reference is the hard vacancy storage/write override: vacancy rows physically live in `Queue`, `Active`, `Low fit`, or `Closed`; `Jobs` is a read-only aggregate; ChatGPT/agent vacancy-row writes are Queue-only and cross-partition movement belongs to the bound UI/Apps Script automation. `salary-normalization-v6.md` is the hard salary override and supersedes conflicting salary text in the skill or older tracker references: salary research/calculation is mandatory before a vacancy may remain `Reviewed` or `CV ready`, regardless of Fit %, and a missing employer-published range requires market research rather than a silent blank. `cv-markdown-v2.md` is the hard CV artifact override: tailored CVs are authored and stored canonically as Markdown; agents write only the verified source URL into Queue `CV`; the bound Queue-only presentation helper converts that source to `DOCX PDF`; lifecycle copy preserves those links in later partitions; DOCX/PDF are on-demand derivatives and are not required to be persisted for `CV ready`. The activity-log reference is the canonical append-only history for email/recruiter/process events linked by immutable Row ID.
- Use `Antiokh/CV` as the primary evidence repository.
- Apply fit scoring, automatic Markdown CV generation above 60%, mandatory normalized salary research, `WorkApplications`, the WorkInterviews tracker, append-only Activity Log, Markdown-first CV storage, Queue-only CV presentation and read-only Gmail status checks.
