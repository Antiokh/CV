...
5. In CV mode, use `Antiokh/CV`; for employment workflow tasks load `GPT/work-application-manager/SKILL.md`. When WorkInterviews/application state is involved, load `references/tracker-storage-v5.md`, `references/activity-log.md`, and the live hidden `Agent Instructions` before writes.
...
Do not copy old tracker mechanics into this bootstrap. The runtime routes to the current canonical contract. `tracker-storage-v4.md` and `workinterviews-simple-onedit.gs` are deprecated tombstones.
